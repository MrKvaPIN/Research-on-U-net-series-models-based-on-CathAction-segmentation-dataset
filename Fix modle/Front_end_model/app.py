...
class RecurrentBlock(nn.Module):
    def __init__(self, out_ch, t=2, bias=False):
        super().__init__()
        self.t = t
...
class RRCNNBlock(nn.Module):
    def __init__(self, in_ch, out_ch, t=2, bias=False):
        super().__init__()
        self.conv_1x1 = nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1, padding=0, bias=bias)
        self.rcnn = nn.Sequential(
            RecurrentBlock(out_ch, t=t, bias=bias),
            RecurrentBlock(out_ch, t=t, bias=bias)
        )
...
class R2UNet(nn.Module):
    def __init__(self, img_ch=3, output_ch=3, t=2):
        ...
        self.rrcnn1 = RRCNNBlock(img_ch, 64, t=t, bias=True)
        self.rrcnn2 = RRCNNBlock(64, 128, t=t, bias=True)
        self.rrcnn3 = RRCNNBlock(128, 256, t=t, bias=True)
        self.rrcnn4 = RRCNNBlock(256, 512, t=t, bias=True)
        self.rrcnn5 = RRCNNBlock(512, 1024, t=t, bias=True)

        self.up5 = UpConv(1024, 512)
        self.up_rrcnn5 = RRCNNBlock(1024, 512, t=t, bias=True)

        self.up4 = UpConv(512, 256)
        self.up_rrcnn4 = RRCNNBlock(512, 256, t=t, bias=True)

        self.up3 = UpConv(256, 128)
        self.up_rrcnn3 = RRCNNBlock(256, 128, t=t, bias=True)

        self.up2 = UpConv(128, 64)
        self.up_rrcnn2 = RRCNNBlock(128, 64, t=t, bias=True)
...
class UpRRCNNAttBlock(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, t=2):
        ...
        self.up = nn.ConvTranspose2d(in_ch, out_ch, kernel_size=2, stride=2)
        self.att = AttentionGate(g_ch=out_ch, x_ch=skip_ch, inter_ch=max(out_ch // 2, 16))
        self.rrcnn = RRCNNBlock(out_ch + skip_ch, out_ch, t=t, bias=False)
...
class AttentionR2UNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=3, base_ch=32, t=2):
        ...
        self.enc1 = RRCNNBlock(in_channels, base_ch, t=t, bias=False)
        self.enc2 = RRCNNBlock(base_ch, base_ch * 2, t=t, bias=False)
        self.enc3 = RRCNNBlock(base_ch * 2, base_ch * 4, t=t, bias=False)
        self.enc4 = RRCNNBlock(base_ch * 4, base_ch * 8, t=t, bias=False)
        self.center = RRCNNBlock(base_ch * 8, base_ch * 16, t=t, bias=False)
...
def infer_model_type(filename, state_dict):
    ...
    if any(k.startswith("up5.") or k.startswith("up_rrcnn5") for k in keys):
        return "r2unet"

    if "attention_r2" in name or ("attention" in name and "r2" in name):
    ...
    if any("rrcnn" in k for k in keys):
        if any("att.W_g" in k or "up_rrcnn" in k for k in keys):
            return "attention_r2unet"
        return "r2unet"
...
def build_model(model_type):
    ...
    if model_type == "r2unet":
        return R2UNet(img_ch=3, output_ch=NUM_CLASSES, t=2), 768
    return AttentionR2UNet(in_channels=3, num_classes=NUM_CLASSES, base_ch=32, t=2), 768
...
with col_left:
    ...
    st.info(f"Model: {model_type} | Device: {device_name} | Input Resolution: {input_res}x{input_res}")
